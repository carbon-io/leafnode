 ...
 * Update a single document.
 * @method
 * @param {object} doc The doc to update (must contain "_id" property).
 * @param {object} update The update operations to be applied to the document.
 * @param {object} [options=null] Optional settings.
 * @param {boolean} [options.upsert=false] Update operation is an upsert.
 * @param {(number|string)} [options.w=null] The write concern.
 ...
 */
Collection.prototype.updateObject = function(doc, update, options) {
  // XXX: should this allow an id to be passed in instead of the full doc?
  if (_.isUndefined(doc._id)) {
    throw new Error('_id is required')
  }
  var ret = sync(this._collection, 'updateOne', [{_id: doc._id}, update, options])
 ...
 * Update multiple documents.
 * @method
 * @param {object} docs The docs to update (must contain "_id" property).
 * @param {object} update The update operations to be applied to the document.
 * @param {object} [options=null] Optional settings.
 * @param {boolean} [options.upsert=false] Update operation is an upsert.
 * @param {(number|string)} [options.w=null] The write concern.
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 * @return {object} The number of documents modified.
 * @throws {Error, LeafnodeObjectSetOperationError}
 */
Collection.prototype.updateObjects = function(docs, update, options) {
  // XXX: should this allow a list of ids to be passed in instead of the list of full docs?
  var query = {_id: {'$in': _.filter(docs, function(doc) { return '_id' in doc })}}
  if (query._id['$in'].length != docs.length) {
    throw new Error('_id is required')
  }
  query._id['$in'] = _.map(query._id['$in'], '_id')
  var ret = sync(this._collection, 'updateMany', [query, update, options])
  if (ret.modifiedCount != docs.length) {
    throw new errors.LeafnodeObjectSetOperationError(
      'UpdateObjects failed to update one or more documents', ret.modifiedCount, docs.length)
  }
}
 ...
 * Delete a single document.
 * @method
 * @param {object} doc The doc to delete (must contain "_id" property).
 * @param {object} [options=null] Optional settings.
 * @param {(number|string)} [options.w=null] The write concern.
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 * @return {object} The number of documents deleted.
 * @throws {Error, LeafnodeObjectSetOperationError}
 */
Collection.prototype.deleteObject = function(doc, options) {
  // XXX: should this allow an id to be passed in instead of the full doc?
  if (_.isUndefined(doc._id)) {
    throw new Error('_id is required')
  }
  var ret = sync(this._collection, 'deleteOne', [{_id: doc._id}, options])
 ...
 * Remove a single document.
 * @method
 * @param {object} doc The doc to remove (must contain "_id" property).
 * @param {object} [options=null] Optional settings.
 * @param {(number|string)} [options.w=null] The write concern.
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 * @return {object} The number of documents removed.
 * @throws {Error, LeafnodeObjectSetOperationError}
 * @deprecated Use {@link deleteObject} 
 */
Collection.prototype.removeObject = function(doc, options) {
  return this.deleteObject(doc, options)
}

/**
 * Delete multiple documents.
 * @method
 * @param {object} doc The docs to delete (must contain "_id" property).
 * @param {object} [options=null] Optional settings.
 * @param {(number|string)} [options.w=null] The write concern.
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 ...
 * @throws {Error, LeafnodeObjectSetOperationError}
 */
Collection.prototype.deleteObjects = function(docs, options) {
  // XXX: should this allow an id to be passed in instead of the full doc?
  // XXX: add utility method to build the query for this and update
  var query = {_id: {'$in': _.filter(docs, function(doc) { return '_id' in doc })}}
  if (query._id['$in'].length != docs.length) {
    throw new Error('_id is required')
  }
  ...
  if (ret.deletedCount != docs.length) {
    throw new errors.LeafnodeObjectSetOperationError(
      'deleteObjects failed to delete one or more documents', ret.deletedCount, docs.length)
  }
}
 ...
 * Remove multiple documents.
 * @method
 * @param {object} doc The docs to remove (must contain "_id" property).
 * @param {object} [options=null] Optional settings.
 * @param {(number|string)} [options.w=null] The write concern.
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 ...
 */
Collection.prototype.removeObjects = function(docs, options) {
  return this.deleteObjects(docs, options)
}
